...
async function main() {
  ...
  const projectData = [
    ...
    },
    {
      ...
      summary: "동시성 제어, 이벤트 기반 아키텍처, 데이터 정합성을 설계하고 구현한 이커머스 백엔드 프로젝트입니다.",
      description: [
        "Clean Architecture 기반 4-Layer 구조(Interfaces → Application → Domain → Infrastructure)로 도메인 로직과 인프라를 분리하고, 서비스 간 책임 경계를 명확히 설계",
        "Redis Lua 스크립트로 재고 확인·차감을 원자적으로 처리하여 동시 100건 요청 테스트에서 오버셀 0건 달성, Sorted Set(ZINCRBY + ZREVRANGE)으로 O(log N) 실시간 랭킹 조회 구현",
        "Kafka 기반 이벤트 아키텍처 + Transactional Outbox 패턴으로 트랜잭션과 이벤트 발행의 원자성을 보장하고, Kafka 장애 시에도 이벤트 유실 없이 재발행 가능한 구조 설계",
        "PG 결제 연동에서 응답 타임아웃·네트워크 오류 시 PENDING 상태 저장 → 스케줄러 기반 PG 재조회 → 보상 트랜잭션(재고 원복 + 주문 CANCELLED)으로 자동 복구하는 Recovery Flow 구현",
        "Idempotency Key 기반 중복 요청 차단, Redis Set(SADD)으로 쿠폰 중복 사용 원자적 방지, @Transactional 경계와 전파 속성(REQUIRES_NEW)을 분리하여 부분 롤백 제어",
        "Spring Batch(Chunk-oriented Processing)로 일간 판매 메트릭 집계 → 랭킹 API 반영 파이프라인 구현, ItemReader/Processor/Writer 단계별 장애 격리",
      ],
      techStack: ["Java", "Spring Boot", "Kafka", "Redis", "PostgreSQL", "REST API"],
      achievement: "동시 100건 오버셀 0건, Outbox 기반 이벤트 무손실, O(log N) 실시간 랭킹",
  ...
  const settings = [
    ...
    {
      key: "about_content",
      value: `불편함을 그냥 넘기지 않는 개발자입니다.

무언가가 비효율적이라고 느끼면, 그걸 **직접 구조화하고 도구로 만들어서 해결**하는 쪽을 선택합니다. 거창한 프로젝트가 아니더라도, 반복되는 작업을 줄이거나 흩어진 정보를 한 곳에 모으는 것만으로도 일하는 방식이 바뀔 수 있다고 믿습니다.

시험 준비 자료가 여기저기 흩어져 있으면 **학습 앱을 직접 만들고**, 팀에서 공유할 도구가 없으면 **웹 앱을 기획부터 배포까지 혼자 완성**합니다. 필요한 것을 기다리기보다 직접 만드는 습관이, 기획·설계·구현·배포를 아우르는 경험으로 이어졌습니다.

해커톤, 졸업논문, 창업 오디션, 개인 프로젝트까지 — 환경은 매번 달랐지만, **문제를 정의하고 결과물로 증명하는 과정**은 항상 같았습니다. 기술 자체보다 그 기술로 무엇을 해결했는지에 집중합니다.`,
    },
    {
      key: "about_interests",
